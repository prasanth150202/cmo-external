  ...
  const BrandForm = ({ onSubmit, submitLabel }: { onSubmit: () => void; submitLabel: string }) => (
    <div className="p-7 bg-indigo-500/5 border border-indigo-500/20 rounded-3xl space-y-5">
      ...
      <div className="flex gap-3 pt-2">
        ...
        <button onClick={() => { setShowCreate(false); setShowEdit(false); }}
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <AnimatePresence>
        {showCreate && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <BrandForm onSubmit={handleCreateBrand} submitLabel="Create Brand" />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {showEdit && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <BrandForm onSubmit={handleUpdateBrand} submitLabel="Update Brand" />
          </motion.div>
        )}
      </AnimatePresence>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          ...
                          <div>
                            ...
                            {accountForm.platform === "META" && metaAccounts.length > 0 ? (
                              <select value={accountForm.account_id}
                                onChange={e => {
                                  const acct = metaAccounts.find(a => a.account_id === e.target.value);
                                  setAccountForm({ ...accountForm, account_id: e.target.value, account_name: acct?.name || "" });
                                }}
                                className="w-full px-3 py-2.5 bg-black/30 border border-white/10 rounded-xl text-white text-sm font-medium focus:outline-none focus:border-indigo-500">
                                <option value="">Select account</option>
                                {metaAccounts.map(a => <option key={a.account_id} value={a.account_id}>{a.name} ({a.account_id})</option>)}
                              </select>
                              ...
                              googleAccountsLoading ? (
                                ...
                                <select value={accountForm.account_id}
                                  onChange={e => {
                                    const acct = googleAccounts.find(a => a.customer_id === e.target.value);
                                    setAccountForm({ ...accountForm, account_id: e.target.value, account_name: acct?.descriptive_name || `Account ${e.target.value}` });
                                  }}
                                  className="w-full px-3 py-2.5 bg-black/30 border border-white/10 rounded-xl text-white text-sm font-medium focus:outline-none focus:border-indigo-500">
                                  <option value="">Select account</option>
                                  {googleAccounts.map(a => (
                                    <option key={a.customer_id} value={a.customer_id}>
                                      {a.descriptive_name ? `${a.descriptive_name} (${a.customer_id})` : a.customer_id}
                                    </option>
                                  ))}
                                </select>
                              ) : (
